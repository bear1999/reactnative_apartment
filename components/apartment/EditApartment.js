...
class EditApartment extends React.Component {
	constructor(props) {
		super(props);
		const { currentType, idMain, currentStatus, Tittle, priceRent } = this.props.route.params;
		this.state = {
			dataApartment: [],
			dataStatus: [],
			//Form lưu vào database
			formData: {
				idType_apartment: currentType,
	...
	saveData = async () => {
		...
		await fetch(host + '/apartment/UpdateApartment.php', {
			method: 'PUT',
			headers: {
						...
						>

							{
								this.state.dataApartment.map((item, index) =>
									idType_apartment == item.idType_apartment ?
