...
class EditService extends React.Component {
	constructor(props) {
		super(props);
		const { name_service, description_service, price_service, unit, type, idType_service, imageTypeService } = this.props.route.params;
		this.state = {
			...
			formData: {
				...
				unit: unit,
				type: type,
				idType_service: idType_service,
				imageTypeService: imageTypeService, //lấy link avatar hiện tại xóa ảnh trên server
			}
		}
	}
	/* Chọn avatar */
	...
	saveData = async () => {
		...
		if (!this.state.formData.name_service) //! => false => != null
		...
		await fetch(host + '/service/EditTypeService.php', {
			method: 'POST',
			headers: {
	...
	render() {
		...
		const sourceUri = this.state.avatarSrc.path ? { uri: this.state.avatarSrc.path } : { uri: host + '/assets/logoService/' + imageTypeService };
		/* End Lưu database */
		return (
			<ScrollView>
