...
class ChangePassword extends React.Component {
  ...
  _savePassword = async () => {
    if (!this.state.formData.Password)
      return Alert.alert('', 'Mật khẩu hiện tại không được trống');
    else if (!this.state.formData.newPassword)
    ...
    try {
      await fetch(host + '/menu/ChangePassword.php', {
        method: 'POST',
        headers: {
        ...
        .then((json) => {
          setTimeout(() => {
            ...
            if (JSON.stringify(json) == 1)
              Alert.alert('', 'Mật khẩu hiện tại của bạn không đúng');
